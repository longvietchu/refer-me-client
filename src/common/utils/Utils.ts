...
export function setGallery(
    data: { image350: string; image65: string; image180: string }[]
) {
    var images: {}[] = [];
    data.map(
        (item: { image350: string; image65: string; image180: string }) => {
            if (item.image350) {
                images.push({
...
export function getTimeRemaining(timeEnd: number) {
    ...
    if (distance < 0) {
        return 'Hết giờ';
    } else return days + ' ' + 'ngày' + ' ' + hours + 'h' + ' ' + minutes + 'p';
}
...
export function timeAccept(utcTime: any) {
    const time: number =
        new Date(utcTime).getTime() / 1000 + 7 * 60 * 60 + 2 * 24 * 60 * 60;
    return convert_datetime(time);
}
...
export function mentionTime(utcTime: any) {
    const time: number =
        new Date(utcTime).getTime() / 1000 + 7 * 60 * 60 + 4 * 24 * 60 * 60;
    return convert_datetime(time);
}
